...
import config
from db_setup_demo import client,get_next_order_id
...
stores = pd.DataFrame(client.query(f"SELECT store_id FROM {CH_DB}.stores where store_id = 105").result_rows, columns=["store_id"])
...
order_counter  = round(get_next_order_id() / 1000)
# --- Order Generator ---
...
while True:
    ...
    order_counter  = round(get_next_order_id() / 1000)
    time.sleep(1)   # send one order every 5 seconds
